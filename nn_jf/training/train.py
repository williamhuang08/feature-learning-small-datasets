...
from nn_jf.models.model import grid_search_nn
from nn_jf.utils.utils import ensure_dir, load_config, save_json, tensor_dtype
from nn_jf.utils.data import list_binary_datasets, load_arff_data, load_metadata, load_validation_split, subset_dataset
...
def train_one_dataset(config: dict, dataset_name: str) -> None:
    ...
    save_json(
        best_params_path,
        {
            ...
            "use_batch_norm": best_result.use_batch_norm,
            "lr": best_result.lr,
    ...
    print(
        ...
        f"layers={best_result.num_layers} "
        f"h_dim={int(config['hidden_dim'])} "
        f"batch_norm={best_result.use_batch_norm} "
        f"lr={best_result.lr}"
    )
